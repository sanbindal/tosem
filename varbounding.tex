...
%\thanks{This is an extended version of a paper by the same title
%presented at ISSTA 2013.
%We have extended our previous paper
%in the following ways:
%(XXX: point to sections)
%\begin{enumerate}
%\item A discussion on the effect of variable bounding on stress testing.
%\item More experimental comparisons of CHESS with Rankchecker on new benchmarks.
%\item A discussion on a systematic deterministic exploration algorithm for thread bounding.
%\item A detailed discussion of the highest-depth bug we found using our tool.
%\end{enumerate}
%}
...
been proposed in this direction. Musuvathi and Qadeer used
{\em context-bounding} to rank schedules, and showed that it is an effective
...
Our first contribution is to use the number of variables to further
classify and reduce the search space. Our algorithm is based on the
hypothesis that in practice most concurrency bugs can be uncovered by restricting
the search to only a few variables at a time. 
...
experimentally by showing that variable bounding reduces time needed to uncover
concurrency bugs.
...
the non-deadlock concurrency bugs that they examined involved only one variable.
...
execute schedules that explore {\em all} interactions between variables
in $V$.
...
prioritized the variables that are generated in the first few loop
iterations.
...
Without variable bounding, all variable accesses  must be instrumented
with a call to a scheduler that implements exhaustive schedule enumeration.
...
it may be accessed $a_{x_i}$. With variable bounding, we only check
interactions within a variable subset
$V=\{x_1,\dots,x_v\}$ and only instrumente
locations in the
set $(a_{x_1} \cup \dots \cup a_{x_v})$.
The instrumentation
code includes a call to a scheduler function, {\tt varaccess()} that
yields to the scheduler that
...
variable, we also insert a {\em fake} {\tt varaccess()} before the first
...
%A randomized testing procedure samples from an otherwise
%impractically huge space of possible execution
%schedules.
%So far, we have focussed on the advantages of
%variable bounding in its 
%one such testing algorithm, namely PCT. Variable bounding is attractive
%for two reasons: firstly it prioritizes schedules based on the number of variables
%involved in the 
%
%We next discuss the effect of variable bounding on other randomized testing algorithms
%that do not necessarily provide probabilistic guarantees on finding bugs.
%
%Repeated
%random testing with a realistic scheduler, also called stress testing, is
%a popular testing method. We show
...
fragments.
To cover all $t$-thread bugs at $c$ context-bound, it suffices to
enumerate all $(t+c)!$ permutations of all
...
We next discuss a deterministic algorithm to explore
bugs in increasing order of their thread-bounds. We
again consider the problem of finding the minimum number of permutations
of $n$ elements required to enumerate all $t!$ permutations of all $\binom{n}{t}$
$t$-sized subsets of the $n$ elements, this time without randomization.

A brute-force algorithm is to enumerate
all $\binom{n}{t}(t)!$ permutations of all $t$-sized subsets of the $n$
elements, and for each permutation thus generated, append the remaining
$n-t$ elements to it in any order. This
enumeration of $\frac{n!}{(n-t)!}$ permutations is significantly
higher than that required for our randomized
algorithm. Notice that there is significant redundancy in
these enumerations as we ignore the permutation orders generated due to
the remaining $n-t$ elements.

We optimize this enumeration by noticing that $t=2$ is a very special case
and
as we discussed in
Section~\ref{sec:threadbound},
only two schedules are required to
enumerate all $2$ permutations of all $2$-sized subsets of any $n$
elements --- the brute-force algorithm would have required $O(n^{2})$
enumerations. Keeping this in mind, we divide any $t$-sized subset
into two subsets of sizes $t1$ and $t-t1$. The total number of permutations
of all $t$ subsets of $n$ elements, can be expressed as a product of
the number of $t1$ permutations of $n$ elements, and the number of
$t-t1$ permutations of the remaining $n-t1$ elements, where $0 < t1 < t$.
In other words,
$$
\binom{n}{t}(t)! = \binom{n}{t1}(t1)! * \binom{n-t1}{t-t1}(t-t1)!
$$
Using $t-t1=2$, and noticing that the expression $\binom{n-t1}{2}(2)!$ can be replaced
by $2$ (by observing that only two permutations are required to enumerate
both permutations of all two-sized subsets of any $n$ elements), we find that at most
$$
2\binom{n}{t-2}(t-2)!
$$
enumerations of $n$ elements are required to enumerate all $t$ permutations of all $t$-sized
subsets of the $n$ elements. While even this involves a significant amount of overcounting,
it is quite useful in practice, as $t$ is usually small.
For example, at $c=0$ and $t=3$, all $3$-thread bugs can be uncovered
by only $2n$ schedules, namely
...
in increasing and decreasing order. For context-bound $c>0$,
the same argument used
in Section~\ref{sec:threadbound-random} of considering
a pre-empted thread as two distinct threads is applied to show that
$2\binom{n+c}{t+c-2}(t-2)!$
permutations suffice to uncover all $t$-thread bugs at context
bound $c$.
...
in \cite{naik:chord:pldi06}\cite{whaley:alias:pldi04}.
Like previous approaches on systematic and probabilistic
testing~\cite{chess} \cite{pct}, the program under test is required
...
variable and thread bounding options in CHESS for testing C\# programs, and we report our
experience on the {\tt RegionOwnership} benchmark.
...
it was accessed by at least two threads. We tested both randomized and
deterministic algorithms for thread bounding; because most bugs had
small thread-bounds, both algorithms resulted in similar
results.
...
We now discuss the improvements in running time due to variable bounding.
Table~\ref{tab:pref_results} shows our results on a subset of the Java programs (others
were too small to show any meaningful improvements). The runtime
...
previous work, where only synchronization operations were considered as
potential pre-emption points~\cite{chess}\cite{pct}.
\begin{table}
\begin{center}
\scriptsize
\setlength{\tabcolsep}{2pt}
\tbl{The different columns in this table represent 
the name of the program, the (average) number of byte code instructions executed by the program, total number of instrumentation sites, 
...
reported this to an author of RaceFuzzer~\cite{sen:racefuzzer:pldi08} and he
...
We also compared RankChecker with CHESS directly, to study the
effectiveness of our
variable and thread bounding algorithms. We ran
RankChecker and CHESS on the same set of programs from the
ConTest benchmark suite,
written
in C\# and Java. Table~\ref{tab:chess_rank_results} shows the number
of schedules explored before the bug was found by CHESS and RankChecker.
RankChecker used the same exhaustive and deterministic search
strategy as CHESS, except that it was augmented with variable and thread
bounding.
...
CHESS and the systematic strategy of RankChecker before finding
the bug and the ($c$,$v$,$t$) value at which bug was found.
...
\begin{tabular}{l|r|r|r}
    ...
    \hline
    LinkedList & 23000 & 29 & (1,$v_{tot}$,2) \\
    \hline
...
{\tt\bf c1.notify();}                  &    \\
{\em first context switch}        &    {\tt\bf c1.wait();} \\
                                  &    {\tt\bf Result = x;}\\
                                  &    {\tt\bf finished = true;}\\
{\tt\bf t1 = GrabResult() \{}       &    {\em second context switch}\\
{\tt\bf ~~if(finished) \{}           &    \\
{\tt\bf ~~~~return Result;}          &    \\
{\tt   ~~\} else \{ }               &    \\
{\tt   ~~~~c2.wait(); // A}                 &    \\
...
{\tt\bf ~~\} else \{ }               &    \\
{\tt\bf ~~~~c2.wait(); // C }            &    \\
{\em~~~~ non-preemptive context switch}&    {\tt\bf c2.notify(); // D} \\
                                  &    {\tt\bf c1.wait();}\\
{\tt\bf ~~~~return Result;}          &    {\em third context switch}\\
{\tt ~~\}}                        &    \\
{\tt \}}                          &    \\
{\tt\bf assert(t2 == t1 + 1); !FAILS! }    &    \\
                                  &    {\tt Result = x+1;}\\
                                  &    {\tt finished = true;}\\
                                  &    {\tt c2.notify(); // E}\\
\end{tabular}
...
\caption{\label{fig:regionownership_bug}Pseudo-code showing the $c=3,v=2,t=2$ bug
in RegionOwnership. 
Executed statements are
...
called {\tt RegionOwnership}, shown in Figure~\ref{fig:regionownership_bug}. It
is a C\# library
...
\subsubsection*{Bug description} Figure~\ref{fig:regionownership_bug} shows the $c = 3, v = 2, t = 2$ bug
in {\tt RegionOwnership}.\footnote{We have changed identifier names from the original program for clarity.}
The second thread does some computation and stores the result in the shared variable {\tt Result}.
The first thread calls the procedure {\tt GrabResult} twice to consume the result from {\tt Result} and expects a
certain invariant to hold between the two values (denoted by the assertion towards the end of the first thread).

The bug requires three context switches. First, notice that 
the {\tt wait} on line C was intended to match with the {\tt notify} on line E, but 
the bug happens because C ends up matching with the {\tt notify} on line D. 
Let us now start reasoning from the beginning of the trace.
It must be that the first thread starts because it sends the initial notification (via {\tt c1}), but it must
context switch before executing line A (otherwise C will not match with D because D will match with A). After the
first context switch, the second thread starts executing but it must context switch before executing D because the
first thread must execute B first (otherwise B will clear D's notification). The third context switch is necessary so
that {\tt Result} still has a stale value and the assertion in the first thread fails.

Also, note that the trace requires a variable bound of $2$; it only preempts at access to {\tt c1} and
{\tt finished}.
...
but Table~\ref{tab:vb-chess} shows a further reduction in search space when this bound is imposed. The condition
variables {\tt c1} and {\tt c2} are used for synchronization. These objects support a {\tt wait}-{\tt notify}-{\tt reset} 
API in the usual sense (the {\tt reset} call clears any previous notification on the condition variable).
...
All bugs found in our test programs, except {\tt RegionOwnership}, were of type $v\leq1$.
...
variable bounding. We wrote a small toy program, shown in
figure~\ref{fig:account-prog}, which spawns 11 threads
with a total of 23 shared variables, and tested it with 
variable-bounding-enabled CHESS.
...
static~\cite{bessey:coverity:acmcomm10}\cite{naik:chord:pldi06} and
dynamic~\cite{savage:eraser:tcs97}\cite{elmas:goldilocks:pldi07}
\cite{joshi:deadlockfuzzer:pldi09}\cite{lu:avio:asplos06}\cite{ctrigger}
...
\begin{enumerate}
  ...
  \item Many tools have false positives. Spending time and energy on
    a false-positive bug report is counter-productive as well as annoying.
...
\cite{bruening:systematic_testing}\cite{chess}\cite{godefroid:verisoft:book96}\cite{godefroid:verisoft:popl97}\cite{stoller:modelcheck:spin00}\cite{khurshid:pathfinder:tacas03}
are
closer
...
VeriSoft~\cite{godefroid:verisoft:book96}\cite{godefroid:verisoft:popl97}\cite{godefroid:verisoft:issta98} also uses an
...
race-detection tools~\cite{savage:eraser:tcs97}\cite{naik:chord:pldi06}\cite{sen:racefuzzer:pldi08}\cite{yu:racetrack:sosp05}\cite{marino:literace:pldi09},
deadlock-detection tools~\cite{joshi:deadlockfuzzer:pldi09}, and
atomicity-violation detection tools~\cite{lu:avio:asplos06}\cite{ctrigger}.
...
systematic testing of concurrent programs. Experiments on a
variety of Java and C\# programs reveals that this ranking significantly
aids early discovery of common bugs.

%\ifthenelse{\boolean{ThisIsTR}}{
%\section{ACKNOWLEDGEMENTS}
\begin{acks}
We thank Sandeep Sen (IIT Delhi) for
